...
**推奨: Fine-grained tokens（最小権限の原則）**

1. GitHub → Settings → Developer settings → Personal access tokens → **Fine-grained tokens**
2. Generate new token
3. Repository access: **Only select repositories** → `Akyodex` を選択
4. Permissions:
...
**代替: Classic tokens（従来方式）**

Classic tokensも使用可能ですが、`repo`スコープは全リポジトリへのアクセス権を付与するため、セキュリティリスクが高くなります。Fine-grained tokensの使用を推奨します。
