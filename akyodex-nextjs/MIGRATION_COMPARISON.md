...
### 3. **Dify AI Chatbot Integration** 🟡 Medium Priority

**Old Implementation:**
- Embedded Dify chatbot widget
- "ずかんAkyoにきく" (Ask Zukan Akyo) button in header
- Configuration via `window.difyChatbotConfig`
- Cloudflare Pages preview hostname detection and user guidance
- Error handling for unavailable chatbot

**Current Status in Next.js:**
- Not implemented (0 references to "dify" or "chatbot" in codebase)

**Migration Recommendation:**
- Add Dify script to Next.js layout if user wants AI assistant feature
- This is a third-party widget integration (low complexity to add)
...
2. **Mini Akyo Background Animation** - Signature visual identity
3. **VRChat Creator Name Extraction** - QoL improvement for data entry

### 🟡 Medium Priority (Nice to Have)

4. **Finder Mode Page** - If distinct from main zukan page
5. **Dify Chatbot** - If AI assistant feature is desired
6. **Service Worker / PWA** - If offline support is desired
7. **ID Auto-Compression Tool** - If ID gaps become problematic
