...
/**
 * VRChat logo (speech bubble with "VRCHAT" text as vector paths)
 *
 * Based on the official VRChat brand logo (Simple Icons).
 * All text is converted to paths for reliable rendering at any size.
 * License: CC0 1.0 Universal (Simple Icons)
 */
export function IconVRChat({ size, className, ...props }: IconProps) {
  return (
    <svg
      ...
      fill="currentColor"
      overflow="visible"
      className={`inline-block ${size ?? 'w-4 h-4'} ${className ?? ''}`}
      aria-hidden="true"
      {...props}
    >
