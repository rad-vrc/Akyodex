...
1. **Query Generation Phase**: When given a topic or problem, you will:
   ...
   **Scenario-Specific Query Strategies (MANDATORY Module Loading)**:
   Before executing any WebSearch or WebFetch, you MUST use the Read tool to load the relevant strategy module(s) from `.agent/skills/deep-research/agents/web-search-modules/`. Based on the research type, read the corresponding file(s):
