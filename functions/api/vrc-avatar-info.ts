...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    if (!(avtr && /^avtr_[A-Za-z0-9-]+$/.test(avtr))) {
      return errJSON(400, "Invalid or missing 'avtr' parameter", {
    ...
    if (!response.ok) {
      return errJSON(
        response.status,
        `Failed to fetch VRChat page: ${response.status}`,
        { headers: corsHeaders(origin) }
      );
    }
    ...
    const titleMatch = html.match(
      /<meta\s+name=["'](?:og:title|twitter:title)["']\s+content=["']([^"']+)["']/i
    );

    if (!(titleMatch && titleMatch[1])) {
      return errJSON(404, "Avatar title not found in VRChat page", {
        headers: corsHeaders(origin),
      });
    }
    ...
    const descMatch = html.match(
      /<meta\s+name=["'](?:description|og:description|twitter:description)["']\s+content=["']([^"']+)["']/i
    );
    ...
    return errJSON(500, `Internal error: ${error?.message || "Unknown"}`, {
      headers: corsHeaders(origin),
    });
  }
};
