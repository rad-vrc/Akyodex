...
async function updateAvatarMapIfNeeded(
  ...
  if (!mapPutRes.ok) {
    throw new Error(
      `Failed to update avatar map: ${mapPutRes.status} ${await mapPutRes.text()}`
    );
  }
...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    // 現行CSVの取得（shaと本文）
    async function getCurrentBase(): Promise<{
      sha?: string;
      text: string;
      eol: "\n" | "\r\n";
    }> {
    ...
    function parseFirstField(line: string): string | null {
      ...
      const n = line.length;
      // handle optional BOM
      if (line.charCodeAt(0) === 0xfe_ff) i = 1;
      if (i >= n) return null;
      if (line[i] === '"') {
        i++;
        let field = "";
    ...
    function parseCsvToRows(text: string): {
      header?: string;
      rows: Array<{ id: string; raw: string; fields: string[] }>;
    } {
    ...
    function parseCsvToMap(
      text: string
    ): Map<string, { raw: string; fields: string[] }> {
    ...
    while (attempt < maxAttempts) {
      ...
      const addedLines = addedIds
        ...
        .map((entry) => entry.raw);
      const allLines = [header, ...keptOrderLines, ...addedLines].filter(
        Boolean
      ) as string[];
      ...
      if (putRes.ok) {
        ...
        try {
          avatarMapUpdated = await updateAvatarMapIfNeeded(
            newBody,
            currentBaseText,
            owner,
            repo,
    ...
    return errJSON(
      500,
      `github update failed after retries: ${lastErrorText || "conflict"}`
    );
  } catch (e: any) {
