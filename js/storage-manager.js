...
class StorageManager {
  ...
  checkIndexedDB() {
    if (!window.indexedDB) {
      console.debug(
        "IndexedDB is not available. Falling back to localStorage."
      );
      return false;
    }
    return true;
    ...
    try {
      ...
      for (const id of imageIds) {
        try {
          ...
          migratedCount++;
          console.debug(
            `Migrated: ${id} (${migratedCount}/${imageIds.length})`
          );
        } catch (error) {
      ...
      console.debug(
        `Migration complete: ${migratedCount} succeeded, ${failedCount} failed`
      );
