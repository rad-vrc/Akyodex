((global) => {
  const defaultDependencies = {
    ...
    getCurrentRole: () => null,
    canDelete: (meta, role) =>
      role === "owner" || (meta ? !!meta.isSession : false),
    onDelete: async () => true,
    onAttributesChanged: () => {},
    buildDeleteMessage: ({ name }) =>
  ...
  const toLocaleLower = (value) =>
    typeof value === "string" ? value.toLocaleLowerCase("ja") : "";

  function createAttributeManagerInstance() {
    const FallbackAttributeName = "未分類";
      ...
      attributes: new Map([
        [
          FallbackAttributeName,
          {
            ...
            isSession: false,
            searchKey: toLocaleLower(FallbackAttributeName),
          },
        ],
      ]),
      fields: new Map(),
    ...
    const tokenizeAttributes = (value) => {
      ...
      if (typeof value === "string") {
        return value.split(AttributeDelimiter);
      }
      return String(value).split(AttributeDelimiter);
    };
    ...
    const parseAttributeString = (value) =>
      normalizeAttributeTokens(tokenizeAttributes(value));

    const serializeAttributes = (value) =>
      normalizeAttributeTokens(tokenizeAttributes(value)).join(
        AttributeDelimiter
      );

    const getFallbackAttributeName = () =>
      state.attributes.has(FallbackAttributeName)
        ? FallbackAttributeName
        : null;
    ...
    function registerBaseField() {
      ...
      if (hiddenInput && badgeContainer && placeholder && button) {
        registerField(
          "add",
          { hiddenInput, badgeContainer, placeholder, button },
    ...
    function ensureDomReferences() {
      ...
      if (!modal) {
        console.warn(
          "[attributeManager] attributeModal not found; attribute features disabled"
        );
        return false;
      }
      ...
      state.dom.closeButtons = Array.from(
        modal.querySelectorAll("[data-attribute-close]")
      );
      ...
      state.dom.createConfirm = document.getElementById(
        "attributeCreateConfirm"
      );
    ...
    function bindModalEvents() {
      ...
      if (state.dom.createStart) {
        state.dom.createStart.addEventListener("click", () =>
          toggleCreateForm(true)
        );
      }
      if (state.dom.createCancel) {
        state.dom.createCancel.addEventListener("click", () =>
          toggleCreateForm(false)
        );
      }
      if (state.dom.createConfirm) {
        state.dom.createConfirm.addEventListener(
          "click",
    ...
    function registerField(fieldId, elements, { initialValue } = {}) {
      ...
      const initial =
        initialValue !== undefined ? initialValue : elements.hiddenInput.value;
    ...
    function handleGridClick(event) {
      ...
      const { attributeName: rawName, attributeAction: action } =
        actionEl.dataset;
      if (!rawName) return;
    ...
    function renderModalList() {
      ...
      items.forEach((meta) => {
        ...
        const canDelete = dependencies.canDelete(
          meta,
          dependencies.getCurrentRole()
        );
        if (canDelete) {
    ...
    function toggleSelection(name, forceState, sourceElement) {
      ...
      const shouldSelect =
        typeof forceState === "boolean" ? forceState : !currentlySelected;
      const fallbackName = getFallbackAttributeName();
      const fallbackWasSelected = fallbackName
        ? state.modalSelection.includes(fallbackName)
        : false;
      ...
      if (fallbackName) {
        ...
        if (
          fallbackWasSelected !== fallbackIsSelected ||
    ...
    async function requestDeleteAttribute(name) {
      ...
      const confirmationMessage = dependencies.buildDeleteMessage({
        name: normalized,
        meta,
        role,
      });
      if (
        confirmationMessage &&
        dependencies.confirmDelete(confirmationMessage, {
          name: normalized,
          meta,
          role,
        }) === false
      ...
      let deleteResult;
      try {
        deleteResult = await dependencies.onDelete({
          name: normalized,
          meta,
          role,
        });
      ...
      if (field.badgeContainer) {
        ...
        if (field.selected.length === 0) {
          ...
          field.selected.forEach((attr) => {
            ...
            badge.className =
              "attribute-badge inline-flex items-center gap-1 text-xs font-semibold shadow-sm";
    ...
    function rebuildFromAkyoData(list) {
        ...
        parseAttributeString(akyo?.attribute).forEach((attr) => {
          ...
          if (existingMeta) {
            if (existingMeta.isSession) {
              const updatedMeta = createMeta(attr, {
                ...existingMeta,
                isSession: false,
              });
      ...
      Array.from(state.attributes.entries()).forEach(([name, meta]) => {
        if (!meta) return;
        const shouldKeep =
          meta.isSession || seen.has(name) || name === FallbackAttributeName;
        if (!shouldKeep) {
      ...
      if (state.attributes.has(FallbackAttributeName)) {
        const fallbackMeta = state.attributes.get(FallbackAttributeName);
        if (fallbackMeta && fallbackMeta.isSession) {
          state.attributes.set(
            FallbackAttributeName,
            createMeta(FallbackAttributeName, {
              ...fallbackMeta,
              isSession: false,
            })
          );
        ...
        state.attributes.set(
          FallbackAttributeName,
          createMeta(FallbackAttributeName, { isSession: false })
        );
        markAttributesDirty();
      }
    ...
    function isModalOpen() {
      return !!(
        state.dom.modal && !state.dom.modal.classList.contains("hidden")
      );
    }
    ...
    function updateModalOptionState(name, sourceElement) {
      ...
      if (
        ...
        sourceElement.dataset.attributeName === normalized
      ) {
      ...
      const indicator = targetButton.querySelector(
  ...
  try {
    Object.defineProperty(global, "attributeManager", {
      value: manager,
      configurable: false,
      writable: false,
    });
    ...
    console.warn(
      "[attributeManager] Unable to define read-only global, falling back to direct assignment",
      error
    );
