...
(() => {
  if (!window.storageManager) {
    console.warn(
      "StorageManager not found. Ensure js/storage-manager.js is loaded before storage-adapter.js"
    );
  }

  // 画像データ読み込み（LocalStorage → IndexedDB の順で試行）
  window.loadImageData = async () => {
    try {
      ...
      if (window.storageManager && window.storageManager.isIndexedDBAvailable) {
        ...
        if (Object.keys(images).length > 0) {
          console.debug(
            `Loaded ${Object.keys(images).length} images from IndexedDB`
          );
          return images;
        }
      }
    ...
    try {
      ...
      if (localData) {
        ...
        console.debug(
          `Loaded ${Object.keys(images).length} images from LocalStorage`
        );
        return images;
      }
    } catch (error) {
  ...
  window.saveImageData = async (imageDataMap) => {
    ...
    if (window.storageManager && window.storageManager.isIndexedDBAvailable) {
      try {
        ...
        if (successCount > 0) {
          console.debug(
            `Saved ${successCount}/${imageIds.length} images to IndexedDB`
          );
  ...
  // 単一画像の保存
  window.saveSingleImage = async (id, imageData) => {
    // IndexedDBへの保存を優先
    if (window.storageManager && window.storageManager.isIndexedDBAvailable) {
      try {
  ...
  // 初期化完了をログ出力
  console.debug(
    "Storage Adapter initialized. Use migrate-storage.html to migrate to IndexedDB for unlimited storage."
  );
})();
