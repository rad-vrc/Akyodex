  ...
  try {
    ...
    if (currentLanguage === "ja") {
      ...
      try {
        ...
        if (headerRowCount) {
          serverCsvRowCount = Number.parseInt(headerRowCount, 10) || 0;
...
function parseCsv(csvText) {
  ...
  rows.forEach((values) => {
    ...
    if (normalized[0] && normalized[0].match(/^\d{3}/)) {
      const [id = "", appearance = "", nickname = "", avatarName = ""] =
        normalized;
      let attribute = "";
      let notes = "";
      let creator = "";
...
function setupEventListeners() {
  ...
  floatingContainer.className =
    "fixed bottom-4 right-4 flex flex-col items-end gap-3 z-50";
