...
const LANGUAGE_CONFIG = {
  ja: {
    ...
    logoAlt: "Akyoずかん",
    title:
      "Akyoずかん-VRChatアバター Akyo図鑑- | Akyodex-VRChat Avatar Akyo Index",
    description:
    ...
    strings: {
      ...
      messages: {
        ...
        partialLoadFailed:
          "一部の読み込みに失敗しました。ページを更新するか再試行してください。",
        initFailed: "初期化に失敗しました。再読み込みしますか？",
        retry: "再試行",
        difyUnavailable:
          "ずかんAkyoにみられている気がする‥ページを再読み込みするか、管理者に連絡してこの不安をぬぐおう。",
        difyPreviewNotice:
          "Cloudflare Pages プレビューでは ずかんAkyoが非表示になる場合があります。Dify 側でプレビューのホスト名を許可してください。",
        difyPreviewHostTitle:
          "Cloudflare Pages プレビューでチャットを表示するには",
        difyPreviewHostBody:
          "Dify 管理画面 → Settings → Website embedding → Allowed domains に次のホスト名を追加してください。",
  ...
  en: {
    ...
    description:
      "Browse more than 500 mysterious Akyo avatars from VRChat. Search by name, creator, or attributes and join the community of Akyo finders!",
    ...
    strings: {
      ...
      messages: {
        ...
        partialLoadFailed:
          "Some content failed to load. Refresh the page or try again.",
        initFailed: "Initialization failed. Reload the page?",
        retry: "Retry",
        difyUnavailable:
          "The AI chat widget did not appear. Refresh the page or review your Dify embed settings.",

        difyPreviewNotice:
          "AI chat can stay hidden on Cloudflare Pages previews. Visit the production domain or allow the preview host in Dify.",
        difyPreviewHostTitle: "Allow Dify chat on this preview environment",
        difyPreviewHostBody:
          "Add the following host name to Dify → Settings → Website embedding → Allowed domains.",
  ...
  try {
    ...
    if (Array.isArray(languages)) {
      if (
        languages.some(
          (lang) =>
            typeof lang === "string" && lang.toLowerCase().startsWith("en-us")
        )
      ) {
        return "en";
      }
  ...
  try {
    ...
    if (twitterImageAlt) {
      twitterImageAlt.setAttribute(
        "content",
        config.twitterImageAlt || config.logoAlt || ""
      );
    }
  } catch (error) {
...
function updateStaticTextContent(lang = currentLanguage) {
  ...
  if (headerAppearance)
    headerAppearance.textContent = strings.listHeaders.appearance;
  ...
  if (headerAttribute)
    headerAttribute.textContent = strings.listHeaders.attribute;
  ...
  if (adminBtn) {
    ...
    adminBtn.setAttribute(
      "aria-label",
      getLanguageConfig(lang).adminButtonTitle
    );
  }
}
...
function updatePreferredLogoPath() {
  ...
  const logoAlt =
    config.logoAlt || (config.code === "en" ? "Akyodex" : "Akyoずかん");
...
function loadFavoritesFromStorage() {
  try {
    ...
    console.warn(
      "Failed to parse favorites from localStorage. Resetting storage.",
      error
    );
    try {
...
function appendVersionQuery(url, versionValue) {
  ...
  return `${str}${str.includes("?") ? "&" : "?"}v=${encodeURIComponent(
    versionValue
  )}`;
}
...
function resolveAkyoImageUrl(akyoId, { size = 512 } = {}) {
  ...
  try {
    if (
      ...
      typeof window.getAkyoVrchatFallbackUrl === "function"
    ) {
  ...
  try {
    ...
    for (const candidate of candidates) {
      try {
        ...
        console.warn(
          "Failed to fetch profile icon candidate:",
          candidate,
          error
        );
      }
...
async function loadImageData() {
  ...
  try {
    ...
    if (!window.storageManager && attempts >= 50) {
      console.debug(
        "StorageManager did not become available within expected time. Falling back."
      );
    }
    ...
    if (window.storageManager && window.storageManager.isIndexedDBAvailable) {
      ...
      console.debug(
        `Loaded ${Object.keys(imageDataMap).length} images from IndexedDB`
      );
      ...
      if (Object.keys(imageDataMap).length === 0) {
        ...
        if (savedImages) {
          try {
            ...
            console.debug(
              `Found ${Object.keys(localImages).length} images in LocalStorage`
            );
      ...
      console.debug(
        "StorageManager not available, loading from LocalStorage..."
      );
      const savedImages = localStorage.getItem("akyoImages");
      if (savedImages) {
        imageDataMap = JSON.parse(savedImages);
        console.debug(
          `Loaded ${Object.keys(imageDataMap).length} images from localStorage`
        );
      }
    }

    console.debug(
      "Image data loaded. Total images:",
...
async function loadAkyoData() {
  try {
    ...
    const ver =
      safeGetLocalStorage("akyoDataVersion") ||
      safeGetLocalStorage("akyoAssetsVersion") ||
      String(Date.now());
    ...
    if (currentLanguage === "ja") {
      ...
      try {
        ...
        try {
          const fallback = await fetch(
            `${csvPath}?v=${encodeURIComponent(ver)}`,
            { cache: "no-cache" }
          );
          if (!fallback.ok)
            throw new Error(`fallback csv failed: ${fallback.status}`);
      ...
      try {
        ...
        if (!fallback.ok)
          throw new Error(`${currentLanguage} csv failed: ${fallback.status}`);
        csvText = await fallback.text();
        loadedFromNetwork = true;
      } catch (langError) {
        ...
        try {
          ...
          const jaResponse = await fetch(
            `${jaPath}?v=${encodeURIComponent(ver)}`,
            { cache: "no-cache" }
          );
    ...
    if (usedJapaneseFallback) {
      console.debug(
        `${currentLanguage}版が未完成のため、日本語版を表示しています`
      );
      // 必要に応じてユーザーに通知
    ...
    if (currentLanguage !== "ja" && !usedJapaneseFallback) {
      try {
        ...
        // 日本語版を取得
        const jaResponse = await fetch(
          `/api/csv?lang=ja&v=${encodeURIComponent(ver)}`,
          { cache: "no-cache" }
        );
        ...
        if (jaResponse.ok) {
          ...
          if (missingEntries.length > 0) {
            console.debug(
              `日本語版から${missingEntries.length}件のデータを補完:`,
            ...
            // 補完があったことを通知（オプション）
            showToast(
              `${missingEntries.length}件のデータを日本語版から補完しました`,
              "info"
            );
          } else {
    ...
    (typeof window !== "undefined" &&
      typeof window.buildSearchIndex === "function"
...
function parseCSV(csvText) {
  ...
  rows.forEach((values) => {
    ...
    if (normalized[0] && normalized[0].match(/^\d{3}/)) {
      let [id = "", appearance = "", nickname = "", avatarName = ""] =
        normalized;
      let attribute = "";
      let notes = "";
      let creator = "";
...
function createCreatorFilter() {
  ...
  const strings = getLanguageStrings();
  const options = Array.from(creatorSet)
    .sort((a, b) => a.localeCompare(b, "ja"))
...
function setupEventListeners() {
  ...
  const safeAreaInset = supportsSafeArea
  ...
  languageBtn.className =
    "bg-orange-500 text-white px-4 py-2 rounded-full shadow-lg hover:bg-orange-400 transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-200";
  ...
  adminBtn.className =
    "bg-gray-800 text-white p-3 rounded-full shadow-lg hover:bg-gray-700 transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300";
  ...
  if (detailModal && !detailModal.dataset.outsideCloseInitialized) {
    const isEventInsideModal = (event) => {
      const modalContentContainer = detailModal.querySelector(
        "[data-modal-content]"
      );
      return modalContentContainer
        ? modalContentContainer.contains(event.target)
        : false;
    };
...
function applyFilters() {
  ...
  if (!currentSearchTerms.length) {
    ...
    result = scored.map(({ id }) => idToAkyo.get(id)).filter(Boolean);
  }
...
function updateQuickFilterStyles() {
  ...
  if (sortBtn) {
    if (sortOrder === "asc") {
      sortBtn.className =
        "attribute-badge quick-filter-badge bg-green-200 text-green-800 hover:bg-green-300 transition-colors";
      sortBtn.innerHTML = `<i class="fas fa-arrow-up-1-9"></i> ${strings.quickFilters.sortAsc}`;
    } else {
      sortBtn.className =
        "attribute-badge quick-filter-badge bg-blue-200 text-blue-800 hover:bg-blue-300 transition-colors";
...
function switchView(view) {
  ...
  if (view === "grid") {
    ...
    document.getElementById("gridViewBtn").className =
      "inline-flex items-center justify-center w-10 h-10 md:w-12 md:h-12 bg-purple-500 text-white rounded-xl";
    document.getElementById("listViewBtn").className =
      "inline-flex items-center justify-center w-10 h-10 md:w-12 md:h-12 bg-gray-300 text-gray-600 rounded-xl";
  } else {
...
function createAkyoCard(state) {
  ...
  favoriteButton.className =
    "absolute top-2 right-2 w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center hover:scale-110 transition-transform";
  ...
  badgeContainer.className =
    "flex flex-wrap gap-1 mb-2 akyo-attribute-container";
  content.appendChild(badgeContainer);

  const creator = document.createElement("p");
  creator.className =
    "text-xs text-gray-600 mb-2 akyo-creator whitespace-pre-line";
  content.appendChild(creator);

  const detailButton = document.createElement("button");
  detailButton.className =
    "detail-button w-full bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500 text-white py-3 rounded-2xl hover:opacity-90 transition-all duration-300 transform hover:scale-105 font-bold text-lg shadow-lg hover:shadow-xl relative overflow-hidden";
...
function updateAkyoCard(card, state) {
  ...
  if (favoriteIcon) {
    favoriteIcon.className = `fas fa-heart ${
      state.isFavorite ? "text-red-500" : "text-gray-300"
    }`;
    const favoriteButton = favoriteIcon.closest("button");
    if (favoriteButton) {
      const label = state.isFavorite
        ? favoriteStrings.remove
        : favoriteStrings.add;
  ...
  if (mediaContent) {
    ...
    if (state.imageUrl) {
      ...
      img.addEventListener("error", () =>
        handleImageError(img, state.id, state.attributeColor, "card")
      );
...
function createListRow(state) {
  ...
  attributeContainer.className =
    "flex flex-wrap gap-1 akyo-attribute-container";
...
function updateListRow(row, state) {
  ...
  if (imageWrapper) {
    ...
    if (state.imageUrl) {
      ...
      img.fetchPriority = "low";
      img.addEventListener("error", () =>
        handleImageError(img, state.id, state.attributeColor, "list")
      );
...
async function showDetail(akyoId) {
  ...
  icon.decoding = "async";
  icon.className =
    "w-10 h-10 rounded-full mr-3 inline-block object-cover border-2 border-purple-400";
  ...
  if (hasImage) {
    ...
    imageContainer.className =
      "h-64 overflow-hidden rounded-3xl bg-gradient-to-br from-purple-100 to-blue-100 p-2";
    ...
    modalImage.addEventListener("error", () =>
      handleImageError(modalImage, akyo.id, attributeColor, "modal")
    );
    ...
    sparkle.className =
      "absolute -top-2 -right-2 w-12 h-12 bg-white rounded-full flex items-center justify-center animate-bounce";
  ...
  nameCard.className =
    "bg-gradient-to-br from-pink-50 to-purple-50 rounded-2xl p-4";
  nameCard.innerHTML = `
  ...
  avatarCard.className =
    "bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-4";
  ...
  attributeCard.className =
    "bg-gradient-to-br from-yellow-50 to-orange-50 rounded-2xl p-4";
  ...
  creatorCard.className =
    "bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-4";
  ...
  if (akyo.notes) {
    ...
    notesSection.className =
      "bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 rounded-3xl p-5";
    ...
    notesWrapper.className =
      "bg-white bg-opacity-80 rounded-2xl p-4 shadow-inner";
  ...
  const favoriteButton = document.createElement("button");
  favoriteButton.className = `flex-1 py-3 rounded-lg font-medium transition-colors ${
    akyo.isFavorite
  ...
  favoriteButton.appendChild(favoriteIcon);
  const favoriteActionLabel = akyo.isFavorite
    ? favoriteStrings.remove
    : favoriteStrings.add;
  ...
  if (sanitizedAvatarUrl) {
    ...
    openButton.className =
      "flex-1 bg-gradient-to-r from-purple-500 to-blue-500 text-white py-3 rounded-lg font-medium hover:opacity-90 transition-opacity";
    openButton.addEventListener("click", () =>
      window.open(sanitizedAvatarUrl, "_blank")
    );
...
function updateStatistics() {
  ...
  if (noFilter && !randomMode && !favoritesOnlyMode) {
    ...
    const lastRefetchAt = Number(
      localStorage.getItem("akyoCsvLastRefetchAt") || "0"
    );
    const now = Date.now();
...
function showToast(message, type = "info", retryHandler) {
  ...
  toast.className =
    "fixed bottom-4 right-4 px-4 py-3 rounded-lg shadow-lg z-50 flex items-center gap-3";
