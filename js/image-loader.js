...
function extractImageEntryInfo(rawValue, explicitId) {
  ...
  let versionNum = null;
  if (versionStr) {
    const parsed = Number.parseInt(versionStr, 36);
...
function loadImagesManifestFromCache() {
  try {
    ...
    if (age > MANIFEST_CACHE_DURATION) {
      ...
      console.log(
        "[image-loader] Using stale manifest cache, will refresh in background"
      );
    }
  ...
  for (const source of candidates) {
    if (Array.isArray(source)) {
      const hit = source.find(
        (entry) => String(entry?.id || "").padStart(3, "0") === id3
      );
      if (hit) {
        cache[id3] = hit;
        return hit;
...
function tryGetManifestUrl(manifest, akyoId, versionValue, versionSuffix) {
  ...
  if (value.startsWith("/") || value.startsWith("images/")) {
    return (
      appendVersionIfNeeded(value, versionValue) || `${value}${versionSuffix}`
    );
  }

  const relative = `/images/${value}`;
  return (
    appendVersionIfNeeded(relative, versionValue) ||
    `${relative}${versionSuffix}`
  );
}
...
function getAkyoImageUrl(akyoIdLike, options = {}) {
  ...
  const size = Math.max(
    32,
    Math.min(4096, Number.parseInt(options.size || "512", 10) || 512)
  );
  const versionValue = getAssetsVersionValue();
  const versionSuffix = versionValue
    ? `?v=${encodeURIComponent(versionValue)}`
    : "";

  // 1) マニフェスト（window側が優先）
  try {
    if (typeof window !== "undefined" && window.akyoImageManifestMap) {
      const manifestUrl = tryGetManifestUrl(
...
function setupImageFallback(imgElement, akyoId, options = {}) {
  ...
  imgElement.onerror = function () {
    try {
      ...
      let index = Number.parseInt(this.dataset.fallbackIndex || "0", 10);
      index++;
...
if (typeof window !== "undefined") {
  ...
  window.getAkyoVrchatFallbackUrl = function getAkyoVrchatFallbackUrl(
    idLike,
    options = {}
  ) {
