...
if (!/^\d{4,}$/.test(nextVersion)) {
  fail(`Invalid version "${nextVersion}". Use a numeric stamp such as 20250927.`);
}
...
function updateFile(relativePath, transform) {
  ...
  if (original === updated) {
    fail(`No changes produced for ${relativePath}. Check that the expected patterns exist.`);
  }
...
updateFile('sw.js', (content) => {
  ...
  return content.replace(cssVersionPattern, `const CSS_VERSION = '${nextVersion}'`)
...
['index.html', 'admin.html', 'finder.html', 'logo-upload.html'].forEach((htmlPath) => {
  updateFile(htmlPath, (content) => {
    if (!kidCssPattern.test(content)) {
      fail(`Could not find versioned stylesheet reference in ${htmlPath}`);
    }
    return content.replace(kidCssPattern, `kid-friendly.css?v=${nextVersion}`);
  });
});
...
updateFile('HOSTING-GUIDE.md', (content) => {
  ...
  if (kidCssBacktickPattern.test(updated)) {
    updated = updated.replace(kidCssBacktickPattern, `\`css/kid-friendly.css?v=${nextVersion}\``);
  }
  if (kidBackgroundPattern.test(updated)) {
    updated = updated.replace(kidBackgroundPattern, `akyo-bg.webp?v=${nextVersion}`);
  }
  if (updated === content) {
    fail('HOSTING-GUIDE.md did not contain versioned asset references to update.');
  }
  return updated;
});
